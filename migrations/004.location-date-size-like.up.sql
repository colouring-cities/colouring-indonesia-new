...
ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS location_title varchar;

ALTER TABLE
    buildings
ADD
    CONSTRAINT buildings_location_title_len CHECK (length(location_title) < 90);
...
ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS date_year smallint;

-- Year built (lower estimate)
ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS date_lower smallint;

-- Year built (upper estimate)
ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS date_upper smallint;

-- Date Source
-- ALTER TABLE buildings ADD COLUMN IF NOT EXISTS date_source varchar;
CREATE TABLE IF NOT EXISTS reference_tables.date_source_classification (
    date_source_id serial PRIMARY KEY,
    description_en VARCHAR(128) NOT NULL,
    description_id VARCHAR(128) NOT NULL
);

TRUNCATE TABLE reference_tables.date_source_classification RESTART IDENTITY CASCADE;

INSERT INTO
    reference_tables.date_source_classification (description_en, description_id)
VALUES
    (
        'Expert knowledge of building',
        'Pengetahuan ahli tentang bangunan'
    ),
    (
        'Expert estimate from image',
        'Perkiraan ahli dari gambar'
    ),
    ('Survey of Indonesia', 'Survei Indonesia'),
    ('Pevsner Guides', 'Panduan Pevsner'),
    (
        'Victoria County History',
        'Sejarah Victoria County'
    ),
    (
        'Local history publication',
        'Publikasi Sejarah Lokal'
    ),
    ('Other publication', 'Publikasi Lainnya'),
    (
        'National Heritage List for England',
        'Daftar Warisan Nasional untuk Inggris'
    ),
    (
        'Other database or gazetteer',
        'Database atau Gazetteer lainnya'
    ),
    ('Historical map', 'Peta Sejarah'),
    (
        'Other archive document',
        'Dokumen Arsip Lainnya'
    ),
    ('Film/Video', 'Film/Video'),
    ('Other website', 'Situs web lain'),
    ('Other', 'Lainnya');
...
ADD
    FOREIGN KEY (date_source) REFERENCES reference_tables.date_source_classification (date_source_id);

-- ALTER TABLE
--     buildings
-- ADD
--     CONSTRAINT buildings_date_source_len CHECK (length(date_source) < 150);

ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS date_source_detail varchar;

ALTER TABLE
    buildings
ADD
    CONSTRAINT buildings_date_source_detail_len CHECK (length(date_source_detail) < 500);

-- Facade date
ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS facade_year smallint;

-- Facade date (upper)
ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS facade_upper smallint;

-- Facade date (lower)
ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS facade_lower smallint;

-- Facade date Source
ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS facade_source varchar;

ALTER TABLE
    buildings
ADD
    CONSTRAINT buildings_facade_source_len CHECK (length(facade_source) < 150);

ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS facade_source_detail varchar;

ALTER TABLE
    buildings
ADD
    CONSTRAINT buildings_facade_source_detail_len CHECK (length(facade_source_detail) < 500);
...
ALTER TABLE
    buildings
ADD
    COLUMN IF NOT EXISTS size_storeys_attic smallint DEFAULT 0;
